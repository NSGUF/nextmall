...
type CourseForm = Omit<Course, "id" | "createdAt" | "updatedAt" | "views" | "isDeleted"> & { id?: string };
...
export default function AdminPage() {
    ...
    const { data: courses = [], refetch, isLoading } = api.course.list.useQuery(
        orderBy ? { orderBy, order } : undefined
    );
    const createCourse = api.course.create.useMutation({ onSuccess: () => refetch() });
    const updateCourse = api.course.update.useMutation({ onSuccess: () => refetch() });
    const deleteCourse = api.course.delete.useMutation({ onSuccess: () => refetch() });
    const deleteMany = api.course.deleteMany.useMutation({ onSuccess: () => refetch() });
    ...
    const {
        ...
        defaultValues: { title: "", description: "", videoUrl: "", coverImage: "", duration: 0, creatorId: "", isPublished: false, collectionId: undefined, tags: [], isFree: true, price: undefined, order: undefined },
    });
    ...
    const openEdit = (course?: any) => {
        ...
        if (course) {
            ...
            reset({ title: "", description: "", videoUrl: "", coverImage: "", duration: 0, creatorId: "", isPublished: false, collectionId: undefined, tags: [], isFree: true, price: undefined, order: undefined });
        }
        onOpen();
    };
    ...
    const columns = useMemo(() => [
        ...
        { accessorKey: "coverImage", header: "封面图", width: 120, cell: ({ row }: { row: any }) => row.original.coverImage ? <img src={row.original.coverImage} alt="cover" style={{ width: 32, height: 32, objectFit: "cover" }} /> : null },
        { accessorKey: "duration", header: "时长(秒)", width: 100 },
        { accessorKey: "views", header: "播放次数", width: 100 },
        { accessorKey: "isPublished", header: "上架", width: 80, cell: ({ row }: { row: any }) => row.original.isPublished ? "是" : "否" },
        { accessorKey: "isFree", header: "免费", width: 80, cell: ({ row }: { row: any }) => row.original.isFree ? "是" : "否" },
        { accessorKey: "collectionId", header: "合集ID", width: 120 },
        { accessorKey: "tags", header: "标签", width: 120, cell: ({ row }: { row: any }) => row.original.tags?.join(', ') },
        {
            id: "action",
            header: "操作",
            width: 180,
        ...
        <Box borderRadius="lg" minHeight="full" p={4} bg="white" boxShadow="xs">
            ...
            <DataTable
                columns={columns.map(col =>
                    col.id === "action"
                        ? {
                            ...col, cell: ({ row }: { row: { original: Course } }) => (
                                <Wrap gap={1} flexWrap="nowrap">
                                    <Button size="2xs" colorScheme="blue" onClick={() => openEdit(row.original)}>编辑</Button>
                                    <Button size="2xs" colorScheme="red" onClick={() => handleDeleteWithConfirm(row.original.id)}>删除</Button>
                                </Wrap>
            ...
            {isOpen && (
                <Box position="fixed" left={0} top={0} w="100vw" h="100vh" bg="blackAlpha.400" zIndex={1000} display="flex" alignItems="center" justifyContent="center">
                    <Box bg="white" p={6} borderRadius="md" minW={400}>
                        <Heading size="md" mb={4}>{editing ? "编辑" : "新增"}课程</Heading>
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <Stack gap={2}>
                                ...
                                <Field.Root>
                                    ...
                                    <Input placeholder="视频地址" {...register("videoUrl", { required: "请输入视频地址" })} />
                                </Field.Root>
                                <Field.Root>
                                    <Field.Label>封面图</Field.Label>
                                    <Input placeholder="封面图URL" {...register("coverImage")} />
                                </Field.Root>
                                <Field.Root invalid={!!errors.duration}>
                                    <Field.Label>时长(秒)</Field.Label>
                                    <Input placeholder="时长" type="number" {...register("duration", { required: "请输入时长", valueAsNumber: true })} />
                                </Field.Root>
                                <Field.Root>
                                ...
                                <Field.Root invalid={!!errors.price}>
                                    ...
                                    <Input placeholder="价格" type="number" step="0.01" {...register("price", { valueAsNumber: true })} />
                                </Field.Root>
                                <Field.Root>
                                ...
                                <Field.Root>
                                    ...
                                    <Input placeholder="标签,多个用逗号分隔" {...register("tags", {
                                        setValueAs: v => typeof v === 'string' ? v.split(',').map((s: string) => s.trim()).filter(Boolean) : v
                                    })} />
                                </Field.Root>
                                <Field.Root>
                                    <Field.Label>排序</Field.Label>
                                    <Input placeholder="排序" type="number" {...register("order", { valueAsNumber: true })} />
                                </Field.Root>
                            </Stack>
